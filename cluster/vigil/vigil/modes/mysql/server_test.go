...
func newTestMySQLServer(t *testing.T, ctx context.Context, tst *tests.T,
	rules []*corev1.Policy_Spec_Rule,
	mysqlAuthz *corev1.Service_Spec_Config_MySQL_Authorization) *testMySQLServer {
	t.Helper()
...
func TestAuthorizationDeny(t *testing.T) {
	...
	ts := newTestMySQLServer(t, ctx, tst, []*corev1.Policy_Spec_Rule{
		{
			Condition: &corev1.Condition{
				Type: &corev1.Condition_Match{
					Match: `ctx.request.mysql.query.query.startsWith("UPDATE")`,
				},
			},
		...
		},
		{
			Condition: &corev1.Condition{
				Type: &corev1.Condition_Match{
					Match: `ctx.request.mysql.prepareStatement.query.startsWith("UPDATE")`,
				},
			},
			Effect: corev1.Policy_Spec_Rule_DENY,
		},
	...
	}, &corev1.Service_Spec_Config_MySQL_Authorization{
		Mode: corev1.Service_Spec_Config_MySQL_Authorization_ALL,
	})

	db, err := sql.Open("mysql", fmt.Sprintf("root:@tcp(127.0.0.1:%d)/mysql", ts.port))
	assert.Nil(t, err, "%+v", err)
	t.Cleanup(func() {
		db.Close()
	})
	...
	{
		_, err := conn.ExecContext(ctx, fmt.Sprintf(
			"INSERT INTO %s (id, status) VALUES (1, 'active');", table))
		assert.Nil(t, err, "%+v", err)
	}

	{
		_, err := conn.ExecContext(ctx, fmt.Sprintf(
			"UPDATE %s SET status = 'inactive' WHERE id = 1;", table))
		assert.NotNil(t, err)
		assert.Contains(t, err.Error(), "Octelium: Unauthorized")
	}

	{
		var status string
		err := conn.QueryRowContext(ctx, fmt.Sprintf(
			"SELECT status FROM %s WHERE id = 1;", table)).Scan(&status)
		assert.Nil(t, err, "%+v", err)
	...
	{
		_, err := conn.ExecContext(ctx, fmt.Sprintf(
			"UPDATE %s SET status = ? WHERE id = ?", table), "inactive", 1)
		assert.NotNil(t, err)
		assert.Contains(t, err.Error(), "Octelium: Unauthorized")
	}

	{
		var status string
		err := conn.QueryRowContext(ctx, fmt.Sprintf(
			"SELECT status FROM %s WHERE id = ?", table), 1).Scan(&status)
		assert.Nil(t, err, "%+v", err)
		assert.Equal(t, "active", status)
	}

	{
		assert.Nil(t, conn.PingContext(ctx))
	}
}
