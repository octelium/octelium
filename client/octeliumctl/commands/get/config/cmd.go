...
const example = `
octeliumctl get config
octeliumctl get cfg
octeliumctl get conf -o json
octeliumctl get conf -o yaml
`

var Cmd = &cobra.Command{
	Use:     "config",
	Short:   "List/get Configs",
	Example: example,
...
func doCmd(cmd *cobra.Command, args []string) error {
	...
	if i.FirstArg() != "" {
		res, err := c.GetConfig(cmd.Context(), &metav1.GetOptions{
			Name: i.FirstArg(),
		})
		if err != nil {
			return err
		...
		printList([]*corev1.Config{res}, nil)

		return nil
	}

	itmList, err := c.ListConfig(cmd.Context(), &corev1.ListConfigOptions{
	...
	if len(itmList.GetItems()) == 0 {
		cliutils.LineInfo("No Configs found\n")
		return nil
	}
...
func printList(itmList []*corev1.Config, listMeta *metav1.ListResponseMeta) {
