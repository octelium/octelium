...
func TestGetError(t *testing.T) {
	{
		err := getError(errors.Errorf("Could not apply the tunnel configuration"),
			daemonv1.Error_NETWORK_CONFIGURATION_FAILED)
		assert.Equal(t, daemonv1.Error_NETWORK_CONFIGURATION_FAILED, err.Code)
		assert.Equal(t, "Could not apply the tunnel configuration", err.Message)
		assert.True(t, err.Retryable)
	}
	{
		err := getError(authenticator.ErrAuthenticationRequired, daemonv1.Error_CONNECTION_FAILED)
	...
	{
		err := getError(authenticator.ErrWebAuthenticationTimedOut, daemonv1.Error_AUTHENTICATION_FAILED)
	...
	{
		err := getError(context.DeadlineExceeded, daemonv1.Error_AUTHENTICATION_FAILED)
		assert.Equal(t, daemonv1.Error_AUTHENTICATION_FAILED, err.Code)
		assert.True(t, err.Retryable)
	}
	{
